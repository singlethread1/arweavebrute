...
void AES_ctx_reset(struct AES_ctx *ctx ) {

	//memset(&state_t, 0, sizeof(state_t));
	//memset(&ctx1->Iv, 0, AES_BLOCKLEN);
	//memset(&ctx1->RoundKey, 0, AES_keyExpSize);
}
...
void AES_CBC_decrypt_buffer(struct AES_ctx *ctx, unsigned char *buf, size_t length) {
	size_t i;
	//Only needs to run for the first block so the rest is commented out
