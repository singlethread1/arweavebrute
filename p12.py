...
def filter_answers(answers_list):
    if os.path.exists('brutted.txt'):
        brutted_answers = set()
        with open('brutted.txt') as f:
            for line in f:
    ...
    for k2 in keys_list[1]:
        for k3 in keys_list[2]:
            for k4 in keys_list[3]:
                currentanswer = ''+k1+k2+k3+k4
                if len(currentanswer) == 58:
                    temp_answers.append(currentanswer)
